...
export default function createMode(sportType: SportType, ftp: number, weight: number, runningTimes: RunningTimes): WorkoutMode {
  switch (sportType) {
    case "bike": return new BikeMode(ftp, weight);
    case "run": return new RunMode(runningTimes);
  }
}
