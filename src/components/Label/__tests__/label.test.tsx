...
describe('<Label>', () => {
  test('for cycling, renders: duration, power, w/kg, %FTP, cadence', () => {
    const interval = intervalFactory.steady({ length: new Duration(100), intensity: 1.25, cadence: 0 });
  ...
  test('for cycling ramp, renders: duration, power-range, %FTP-range, cadence', () => {
    const interval = intervalFactory.ramp({ length: new Duration(100), startIntensity: 0.5, endIntensity: 1.0, cadence: 0 });
    const mode = createMode("bike", 200, 75, [], "time");
    const component = renderer.create(
      <Label interval={interval} mode={mode} onCadenceChange={() => {}} />
    );
  ...
  test('for running, renders: duration, distance, %pace, pace type', () => {
    const interval = intervalFactory.steady({ length: new Duration(100), intensity: 1.25, cadence: 0, pace: PaceType.tenKm });
  ...
  test('for running ramp, renders: duration, distance, %pace-range, pace type', () => {
    const interval = intervalFactory.ramp({ length: new Duration(100), startIntensity: 0.5, endIntensity: 1.0, cadence: 0, pace: PaceType.tenKm });
    const mode = createMode("run", 1234, 75, [0, 0, 1000, 0, 0], "time");
    const component = renderer.create(
      <Label interval={interval} mode={mode} onCadenceChange={() => {}} />
    );
